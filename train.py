...
def run(rank, args):
    ...
    # Model
    model = LAE(args.size, nx, args.nh, args.nz, args.step_size, args.n_steps, args.mh).to(rank)
    ...
    if args.log_dir is None:
        log_dir = f"{args.dataset}-{args.size}-LAE-nh{args.nh}-nz{args.nz}-step_size{args.step_size}-n_steps-{args.n_steps}-{'mh' if args.mh else 'no_mh'}-lr{args.lr}-epoch{args.epoch}-seed{args.seed}"
    else:
